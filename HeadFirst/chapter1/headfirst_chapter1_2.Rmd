...
원시 데이터 그 자체는 숫자의 나열일 뿐이며 겉으로 보는 것만으로는 파악하기 어려운 경우가 많습니다. 통계의 다양한 시각화(visualize) 방법을 통해 데이터의 핵심적인 내용을 꿰뚫어 볼 수 있습니다.

우리는 통계로 원시 데이터에 의미를 부여할 수도 있습니다. 이 말은 통계적으로 재구성된 데이터에는 해석을 유도하는 의도가 숨어있을 수 있음을 말해줍니다. 동일한 데이터일지라도 자료 해석과 시각화 방법에 따라 다른 해석이 가능하기 때문이지요.

이해를 돕기 위해 예시를 들어보겠습니다. d는 7월부터 12월까지 매출을 나타낸 데이터입니다. 같은 데이터로도 y축 범위를 살짝 변경하면 다음과 같이 전혀 다른 그래프를 만들 수 있습니다.
